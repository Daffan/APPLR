...
class DuelingDQN(nn.Module):
    def __init__(self, state_shape, action_shape, hidden_layer = [64, 64], cnn = False):
        super().__init__()
        if cnn:
...
net = DuelingDQN(state_shape, action_shape, hidden_layer = training_config['hidden_layer'], cnn = False)
optim = torch.optim.Adam(net.parameters(), lr=training_config['learning_rate'])
...
def delect_log():
    for dirname, dirnames, filenames in os.walk('/home/gauraang/log'):
        for filename in filenames:
            p = join(dirname, filename)
            if p.endswith('.log') and dirname != '/home/gauraang/log':
                os.remove(p)
...
print(training_config['epoch'])
print(training_config['step_per_epoch'])
print(training_config['collect_per_step'])
